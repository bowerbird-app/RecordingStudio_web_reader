...
class RecordingStudioWebReaderTest < Minitest::Test
  def test_version_matches_release
    assert_equal "0.1.0", ::RecordingStudio::WebReader::VERSION
  end

  def test_engine_exists
    assert_kind_of Class, ::RecordingStudio::WebReader::Engine
  end
  ...
  def test_gemspec_excludes_cursor_config
    spec = Gem::Specification.load(File.expand_path("../recording_studio_web_reader.gemspec", __dir__))
  ...
  def test_cursor_environment_is_repo_managed_without_snapshot
    ...
    assert_equal "recording-studio-web-reader", json["name"]
  ...
  def test_template_does_not_ship_copied_core_hooks_or_base_service
    ...
    refute File.exist?(File.expand_path("../lib/recording_studio_web_reader/services/base_service.rb", __dir__))
    refute File.exist?(File.expand_path("../lib/recording_studio_web_reader/services/example_service.rb", __dir__))
  end

  def test_reader_does_not_ship_a_product_capability
    refute File.exist?(File.expand_path("../lib/recording_studio_web_reader/capabilities/example.rb", __dir__))
    refute_includes File.read(File.expand_path("dummy/app/models/workspace.rb", __dir__)), "Capabilities::Example"
  end
  ...
  def test_dummy_home_page_uses_demo_title_only
    ...
    assert_includes view_source, 'title: "Web reader"'
    assert_includes view_source, 'subtitle: "Fetch a public page and inspect the normalized result."'
  ...
  def test_engine_does_not_ship_a_home_view
    view_path = File.expand_path("../app/views/recording_studio_web_reader/home/index.html.erb", __dir__)
