...
require "fileutils"
require "tmpdir"
require "generators/recording_studio_web_reader/install/install_generator"

class InstallGeneratorTest < Minitest::Test
  INSTALL_TEMPLATE_PATH = File.expand_path(
    "../lib/generators/recording_studio_web_reader/install/templates/INSTALL.md",
    __dir__
  )
  ...
  def build_generator(destination_root, options = {})
    RecordingStudio::WebReader::Generators::InstallGenerator.new(
      [],
      options,
  ...
  def test_mount_engine_uses_configured_mount_path
    ...
    assert_equal ["mount RecordingStudio::WebReader::Engine, at: \"/addons/recording\""], routes
  end
  ...
  def test_add_tailwind_source_does_not_duplicate_existing_entries
    with_temp_app do |dir|
      ...
      File.write(css_path, <<~CSS)
        ...
        @source "../../../../../../usr/local/bundle/ruby/**/bundler/gems/recording_studio_web_reader-*/app/views/**/*.erb";
  ...
  def test_install_guide_includes_initializer_and_migration_steps
    install_guide = File.read(INSTALL_TEMPLATE_PATH)

    assert_includes install_guide, "config/initializers/recording_studio_web_reader.rb"
    assert_includes install_guide, "bin/rails generate recording_studio_web_reader:migrations"
