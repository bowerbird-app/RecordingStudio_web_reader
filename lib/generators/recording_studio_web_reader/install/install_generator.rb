  ...
  module WebReader
    module Generators
      class InstallGenerator < Rails::Generators::Base
        namespace "recording_studio_web_reader:install"
        source_root File.expand_path("templates", __dir__)

        desc "Installs Recording Studio Web Reader into your application"

        class_option(
          :mount_path,
          type: :string,
          default: "/recording_studio_web_reader",
          desc: "Route prefix used when mounting the engine"
        )

        def mount_engine
          route %(mount RecordingStudio::WebReader::Engine, at: "#{options[:mount_path]}")
        end

        def copy_initializer
          template "recording_studio_web_reader_initializer.rb", "config/initializers/recording_studio_web_reader.rb"
        end

        def add_yaml_config
          prompt = "Would you like to add `config/recording_studio_web_reader.yml` " \
                   "for environment-specific settings? [y/N]"
          return unless yes?(prompt)

          template "recording_studio_web_reader.yml", "config/recording_studio_web_reader.yml"
        end
        ...
        def add_tailwind_source
          ...
          if missing_lines.empty?
            say "Tailwind already configured to include Recording Studio Web Reader and FlatPack sources.", :green
            return
          end
        ...
        def inject_tailwind_sources(tailwind_css_path, missing_lines)
          ...
          say "Added Recording Studio Web Reader and FlatPack sources to Tailwind CSS configuration.", :green
        ...
        def tailwind_source_lines
          [
            '@source "../../vendor/bundle/**/recording_studio_web_reader/app/views/**/*.erb";',
            "@source \"../../../../../../usr/local/bundle/ruby/**/bundler/gems/" \
            "recording_studio_web_reader-*/app/views/**/*.erb\";",
